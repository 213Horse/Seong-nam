$('.tourism__photos__detail').owlCarousel({
...
function checkButtonState() {
  ...
  if (currentIndex === 0) {
    ...
    document.querySelector('.tourism__prev__post').innerHTML = ""
  } else {
   let prevText = carouselData.items()[currentIndex-1][0].innerHTML
   const match = prevText.match(/<h2>(.*?)<\/h2>/);

    document.querySelector('.tourism__prev__post').innerHTML = match[1]
  ...
  if (currentIndex === itemCount - 1) {
    ...
    document.querySelector('.tourismn__next__post').innerHTML = ""

  } else {
    let nextText = carouselData.items()[currentIndex+1][0].innerHTML
   const match = nextText.match(/<h2>(.*?)<\/h2>/);

    document.querySelector('.tourismn__next__post').innerHTML = match[1]
